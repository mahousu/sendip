...
static void print_usage(void) {
	...
	printf(" -t time\tdelay time in microseconds between each loop run (0 means as fast as possible)\n");
	printf(" -v\t\tbe verbose\n");
	...
	printf(" cN\tto generate a counter (4 bytes), incremented with each call;\n");
	...
	printf("Time delays are specified in microseconds; on systems which\n");
	printf("support nanosleep(), fractional parts are allowed.\n");
	printf("As usual, such delays are in addition to any other processing\n");
	printf("or system delays.\n");
...
int main(int argc, char *const argv[]) {
	...
	struct timespec nanodelay, nanoleft;
#else
	/* Type is useconds_t, however manual entry says use unsigned int */
	unsigned int usecdelay;
	unsigned int secdelay;
#endif
	...
	while(gnuoptind<argc && (EOF != (optc=gnugetopt(argc,argv,"-p:l:t:vd:hf:")))) {
		...
		case 't':
			++dodelay;
#ifdef __USE_POSIX199309
			/* Delays are specified in microseconds; convert
			 * to nanoseconds.
			 */
			nanodelay.tv_nsec =
				(long)round(1000.0 * atof(gnuoptarg));
			if (nanodelay.tv_nsec >= 1000000000) {
				nanodelay.tv_sec =
					nanodelay.tv_nsec/1000000000;
				nanodelay.tv_nsec =
					nanodelay.tv_nsec%1000000000;
			} else {
				nanodelay.tv_sec = 0;
...
#else
			usecdelay = atoi(gnuoptarg);
			/* On some systems, usleep can only take values up
			 * to 1000000. Unfortunately, no method is given for
			 * determining what sort of system this is, other
			 * than trying and failing ... So instead, we'll
			 * just break it into sleep() and usleep().
			 */
...
/*@@ looping */
if (loopcount && dodelay) {
#ifdef __USE_POSIX199309
	/* We don't really care about the "leftover" part */
	nanosleep(&nanodelay, &nanoleft);
#else
	if (secdelay)
		sleep(secdelay);
	if (usecdelay)
