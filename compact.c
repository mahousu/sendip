...
/* Return a 32-bit counter value. For right now, there's a
 * single counter which always starts at 1. Something more
 * sophisticated may be indicated in the future.
 */

u_int32_t
counterargument(void)
{
...
stringargument(char *input, char **output)
{
	...
	/* Special case for rN, zN, fN, tN, cN strings */
	switch (*input) {
	case 'r':
		if (isdigit(*(input+1))) {
			length = atoi(input+1);
	...
	case 't':	/* timestamp */
		...
		break;
	case 'c':	/* Counter argument */
		{
			/* Print out the 4-byte counter value
			 * at the start of the string.
			 */
			static char answer[MAXRAND];

			sprintf(answer, "%u", counterargument());
			*output = answer;
		}
...
integerargument(const char *input, int length)
{
	...
	case 'c':	/* Counter argument */
		{
			u_int32_t counter = htonl(counterargument());

			switch (length) {
			case 1:
				return (u_int8_t)counter;
			case 2:
				return (u_int16_t)counter;
			case 3:
				return (htonl(0xffffff) & counter);
			default:
				return counter;
			}
...
hostintegerargument(const char *input, int length)
{
	...
	case 'c':	/* Counter argument */
		{
			u_int32_t counter = counterargument();
